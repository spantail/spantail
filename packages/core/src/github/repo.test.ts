...
it("strips embedded credentials from https remotes", () => {
	expect(normalizeRemoteUrl("https://user:token@github.com/acme/repo.git")).toBe(
...
it("extracts lowercased full names from github.com URLs only", () => {
	expect(repoFullNameFromUrl("git@github.com:Acme/Spantail.git")).toBe("acme/spantail");
	...
	expect(repoFullNameFromUrl("https://github.example.com/acme/repo")).toBeNull(); // GHES: v1 out of scope
...
it("matches the first mapped remote in array order", () => {
	...
	expect(
		matchRemoteToMapping(
			["https://gitlab.com/x/y", "git@github.com:acme/other.git", "https://github.com/acme/spantail"],
			mapped,
		),
	).toEqual({ fullName: "acme/other" });
	expect(matchRemoteToMapping(["https://github.com/no/match"], mapped)).toBeNull();
});
