...
it("lists spans with project slugs and totals on stderr", async () => {
	const stub = api([
		spanFixture({
			spanDate: "2026-06-12",
			durationMinutes: 90,
	...
	expect(await runCli(["spans", "list"], ctx)).toBe(0);
	...
	expect(stderr.text()).toBe("2 spans, total 2h\n");
});
...
it("sends the default limit and the flag filters", async () => {
	...
	const query = stub.calls.find((call) =>
		call.url.pathname.endsWith("/work-spans"),
	)?.url.searchParams;
