...
-- wrote it; a rollout-window row (inserted by a pre-0017 Worker after the
-- backfill ran) is resolved by content equality exactly like 0017 did, picking
-- the highest matching version for determinism across repeated legacy bodies.
-- If neither resolves — a state the cascade rules make unreachable — the NOT
-- NULL constraint aborts this INSERT before the DROP below, leaving the old
-- table intact: the migration fails loudly instead of losing data.
...
INSERT INTO `__new_report_deliveries`("id", "report_content_id", "sender_user_id", "recipient_user_id", "batch_id", "sender_name", "sender_email", "message", "read_at", "created_at") SELECT "id", COALESCE("report_content_id", (
