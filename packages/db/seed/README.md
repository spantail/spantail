...
`db:reset` is the usual span point: it drops local state, recreates the schema
...
- Members log Mon–Fri for the last 45 days, but the days have texture: hours rise
  ...
  and each span uses a concrete, project-specific task phrase. All of it is
...
- **Daily reports.** Each weekday a member files a per-workspace daily report
  ...
  **cross-workspace daily report** spanning Acme + Globex. A cross-workspace
  report mixes those workspaces' spans, so it is delivered only to people who
...
on load. Activity (work spans, reports, deliveries, shares) is derived at run
