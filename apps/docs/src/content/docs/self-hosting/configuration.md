...
| `BETTER_AUTH_URL` | Recommended | The base URL the app is served from (e.g. `https://spantail.example.com`; `http://localhost:5173` in dev). Used for OAuth callbacks and links. |
...
When you set a provider's credentials, register its callback URL with the provider:
