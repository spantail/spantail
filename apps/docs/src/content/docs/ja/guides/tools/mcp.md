...
ツールを通じて、作業エントリの作成・管理、検索、エージェント活動の閲覧、レポートの作成・閲覧を
あなたのインスタンスに対して行えます。MCP は同じ REST API のクライアントなので、トークンと
同じ権限に従います。

ツールセットは [CLI](/ja/guides/tools/cli/) より意図的に絞っています。対人的な副作用を
持つ操作 — レポートの送信、共有リンク、コメント、受信トレイ — は人が起点になります。
...
しかできず、MCP ツールの呼び出しには使えません。[エージェント活動を記録する](/ja/guides/capturing-agents/)
を参照してください。
...
| `log_work` | 作業エントリを作成。 |
| `log_work_batch` | 最大 100 件の作業エントリを 1 回のアトミックなリクエストで作成（全件か 0 件か）。`externalId` 付きのエントリは重複せずアップサートされます。 |
| `list_entries` | 作業エントリを一覧（任意のフィルタ付き）。 |
| `update_entry` | 自分の作業エントリを更新。 |
| `delete_entry` | 自分の作業エントリを削除。 |
| `search` | 作業エントリとレポートをテキストで検索。 |
...
| `list_agents` | ワークスペース内の自分の AI エージェントを一覧（agent ID の解決用）。 |
| `get_agent_stats` | 期間指定の AI エージェント活動の集計（合計・日別・エージェント別）。 |
| `list_agent_entries` | AI エージェントのセッションを一覧（時間・トークン使用量・コンテキスト付き）。 |
...
| ツール | 動作 |
|---|---|
| `import_work_entries` | ローカルの JSONL ファイルから作業エントリを一括インポート（形式と挙動は [`spantail entries import`](/ja/guides/tools/cli/) と同じ）。 |
