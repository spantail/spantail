...
ワークスペースとプロジェクトを選ぶと（可能なら MCP サーバー経由で候補を提示）、
個人用・gitignore 対象の `.spantail/config.local.json` に書き込まれます。チームで
...
`apiUrl` やトークンを読みません。環境変数（`SPANTAIL_WORKSPACE_ID`、
`SPANTAIL_PROJECT_ID`）は両ファイルより優先されます。

リンクしていないリポジトリは、エージェントトークンの既定ワークスペースに
プロジェクトなしでフォールバックします。各設定がどこから解決されているか、
...
| 名前 | 用途 |
|---|---|
| `/spantail:log-work` | 作業エントリを作成 — 指示した内容からも、現在のセッションの作業内容からも。 |
| `/spantail:log-work #N <duration> [date]` | GitHub Issue に対して記録します。サーバーがリポジトリのマッピング（[GitHub 連携](/ja/admin/github-integration/)参照）からプロジェクトを解決し、Issue の件名とラベルを補完し、関連するエージェントセッションを紐づけます。 |
...
`/spantail:doctor` は MCP なしで動作し、`/spantail:link` は MCP が使えないときは
ID の手入力にフォールバックします。
