...
`workspaceId` と `projectId` は省略時にトークンのバインドが既定値になります。ボディとトークンの
どちらもワークスペースを与えない場合、リクエストは拒否されます。`entryDate` は送りません。
...
`workspaceId` と `projectId` は省略時にトークンのバインドが既定値になり、サマリ経路と同じライブの
委任チェックが適用されます。
