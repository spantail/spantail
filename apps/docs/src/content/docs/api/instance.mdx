...
Whether realtime SSE updates are on. Off by default because every open stream keeps a per-user
Durable Object running, which can exhaust the Workers Free plan's daily duration quota.

### Realtime enabled

`GET /api/v1/instance/realtime-enabled` <Badge text="public" variant="tip" />
...
`PATCH /api/v1/instance/realtime` <Badge text="scope: admin" variant="danger" />
...
`GET /api/v1/instance/version` <Badge text="auth required" variant="note" />
