...
Connecting a GitHub account lets you log work from GitHub issues (see
[GitHub integration](/admin/github-integration/)). The link itself is created by the OAuth
callback in the browser; these endpoints read and remove it.

### Get the GitHub link

`GET /api/v1/me/github` <Badge text="scope: read" variant="note" />

Returns whether the caller's account is linked to a GitHub identity, and the linked login when it
is.
