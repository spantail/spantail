---
title: Workspaces
description: Read, create, update, archive, and delete workspaces, and manage their logo.
---
...
<Aside type="note">
An **archived workspace is read-only**: every write into it — work entries, agent ingest,
projects, members, settings, logo — returns `409 conflict` until it is restored. Reads keep
working and the workspace stays in list responses (`archivedAt` is set). While archived, this
route accepts only the `archived` field; restore first to change anything else. Deleting the
workspace also stays possible.
</Aside>
...
<Aside type="caution">
Deletion cascades: the workspace's members, projects, work entries, agent sessions and events, and
agent tokens bound to the workspace are all permanently removed. Report snapshots that already
rendered its data survive, but the deleted workspace drops out of report scope validation.
</Aside>
