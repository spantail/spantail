...
Returns the [agent sessions](/api/agent-ingest/) linked to this entry — the ones it was logged
from. Same read access as the entry itself. Gated by the instance's **AI agents** feature: when
that is off, the route is unavailable.
