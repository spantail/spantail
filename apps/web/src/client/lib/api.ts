...
export const api = new SpantailClient({
	...
	fetch: async (input, init) => {
		...
		// navigator default) so the server localizes responses — report date
		// formatting, the lazily seeded default template — to the UI language
		// rather than the raw browser Accept-Language header.
