 ...
 * recipient of this content version). Non-participants get a 404 — the
 * version's existence is never revealed, matching requireReportOwner.
 * Mutations additionally require the version to have been shared (≥1
 * delivery); the owner of an unsent version can read the empty discussion but
 * not write to it.
 */
async function requireParticipant(
	...
	const access = await getReportDiscussionAccess(
		c.var.db,
		reportContentId,
		user.id,
	);
...
// Discussion endpoints are content-version-scoped (/report-contents/:id/…): a
// thread belongs to the version that was sent, so recipients of different
// versions never share one, and a new version starts an empty thread.
export const reportDiscussionRoutes = new Hono<AppEnv>()
	.get("/:id/discussion", async (c) => {
		const reportContentId = c.req.param("id");
		const { user } = requireScope(c, "read");
		// A participant (owner or a recipient of this version) reads the thread
		// directly; an admin who is neither still reads it under the owning
		// report's read access (matrix R/R*). Writes stay participant-only
		// (requireParticipant).
		const access = await getReportDiscussionAccess(
			c.var.db,
			reportContentId,
		...
		if (access) {
			...
			const reportId = await getReportIdForContent(c.var.db, reportContentId);
			if (!reportId) throw new AppError("not_found", "Report not found");
			await requireReportReadAccess(c, reportId);
			shared = await isContentShared(c.var.db, reportContentId);
		}
		const [comments, reactions] = await Promise.all([
			listReportComments(c.var.db, reportContentId),
			listReportReactions(c.var.db, reportContentId),
		]);
	...
	.put("/:id/reactions", async (c) => {
		const reportContentId = c.req.param("id");
		const { user } = await requireParticipant(c, reportContentId, "write");
		...
		const reactions = await listReportReactions(c.var.db, reportContentId);
		publishToReportParticipants(c, reportContentId);
	...
	.patch("/:id/comments/:commentId", async (c) => {
		...
		// Missing, wrong version, or not the author — never reveal which.
	...
	.put("/:id/comments/:commentId/reactions", async (c) => {
		...
		const commentId = c.req.param("commentId");
		const { user } = await requireParticipant(c, reportContentId, "write");
		// Validate the comment belongs to this content version so a reaction
		// can't target another version's comment.
		const comment = await getReportComment(
			c.var.db,
			commentId,
			reportContentId,
		);
