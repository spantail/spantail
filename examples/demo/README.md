...
[`r2/`](./r2) mirrors the uploads bucket 1:1 — an avatar per user
(`avatars/<userId>`) and a logo per workspace (`workspaces/<id>/logo`),
extensionless and WebP, named by the seed's deterministic ids. It holds **only**
bucket objects (no docs), so a single sync reproduces the bucket:

```sh
aws s3 sync examples/demo/r2 s3://<bucket> --content-type image/webp
```

`pnpm db:seed` uploads them to the local bucket automatically. The shipped
artwork is a license-free placeholder generated by `pnpm generate-avatars demo`
([`generate-avatars.ts`](../../packages/db/seed/generate-avatars.ts)) — a
stylized silhouette per user and a monogram per workspace, distinct from the
app's initials fallback. Swap any file for a real photo with no code change;
only its object key matters. See
[`packages/db/seed/README.md`](../../packages/db/seed/README.md#r2-assets) for
the full remote-seeding flow.
